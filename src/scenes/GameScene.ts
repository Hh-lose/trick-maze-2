...
type Interactable = { kind: "chest" | "seal" | "exit" | "medpack"; position: Phaser.Math.Vector2; graphic: Phaser.GameObjects.Graphics; opened?: boolean; progress?: number; };
...
export class GameScene extends Phaser.Scene {
  ...
  private attack(target: Enemy): void { this.attackCooldown = Math.max(250, 750 / (1 + this.player.stats.attackSpeed)); const weapon = WEAPONS[this.player.weapon]; const range = this.effectiveAttackRange() * TILE; const crit = this.rng.next() < this.player.stats.crit; const hit: Enemy[] = []; for (const enemy of this.enemies) { const distance = enemy.position.distance(this.player.position); const direction = enemy.position.clone().subtract(this.player.position).normalize(); const dot = direction.dot(this.player.aim); const circle = this.player.weapon === "八卦盘" || this.player.weapon === "拂尘"; const line = this.player.weapon === "红葫芦"; if (distance <= range && (circle || (line ? dot > .86 : dot > .35))) hit.push(enemy); }
    ...
    this.drawAttackEffect(weapon, target, range); [...new Set(hit)].forEach((enemy) => { const amount = damage(this.player.stats.attack, 0, weapon.multiplier, crit); this.damageEnemy(enemy, amount, crit, this.player.weapon === "红葫芦"); }); }
  private drawAttackEffect(weapon: (typeof WEAPONS)[WeaponId], target: Enemy, range: number): void {
    const effect = this.add.graphics().setDepth(15), origin = this.player.position, end = origin.clone().add(this.player.aim.clone().scale(range));
    if (weapon.id === "桃木剑") { const left = origin.clone().add(this.player.aim.clone().rotate(-Math.PI / 4).scale(range)); const right = origin.clone().add(this.player.aim.clone().rotate(Math.PI / 4).scale(range)); effect.fillStyle(0xe9db98, .32); effect.fillTriangle(origin.x, origin.y, left.x, left.y, right.x, right.y); effect.lineStyle(3, 0xf8f2cb, .9); effect.lineBetween(origin.x, origin.y, left.x, left.y); effect.lineBetween(origin.x, origin.y, right.x, right.y); }
    if (weapon.id === "雷符") { effect.lineStyle(3, 0x83e8ef, .8); effect.lineBetween(origin.x, origin.y, target.position.x, target.position.y); effect.fillStyle(0x83e8ef, .18); effect.fillCircle(target.position.x, target.position.y, 1.5 * TILE); effect.lineStyle(3, 0xd9ffff, .9); effect.strokeCircle(target.position.x, target.position.y, 1.5 * TILE); }
    if (weapon.id === "八卦盘") { effect.lineStyle(4, 0xe7c76f, .9); effect.strokeCircle(origin.x, origin.y, range); effect.lineStyle(2, 0xfff1a8, .8); effect.strokeCircle(origin.x, origin.y, range * .58); }
    if (weapon.id === "红葫芦") { effect.lineStyle(TILE, 0xce533a, .22); effect.lineBetween(origin.x, origin.y, end.x, end.y); effect.lineStyle(3, 0xffa16e, .9); effect.lineBetween(origin.x, origin.y, end.x, end.y); }
    if (weapon.id === "拂尘") { effect.lineStyle(5, 0xe8e3cf, .9); effect.strokeCircle(origin.x, origin.y, range); effect.lineStyle(2, 0x8ad6d0, .7); effect.strokeCircle(origin.x, origin.y, range * .7); }
    this.tweens.add({ targets: effect, alpha: 0, duration: 160, onComplete: () => effect.destroy() });
  }
  ...
  private updateEnemies(delta: number): void { this.spawnTimer -= delta; if (this.spawned < this.target && this.spawnTimer <= 0 && this.enemies.length < 55) { this.spawnWave(); this.spawnTimer = 3000; } const combatActive = this.enemies.length > 0 || this.spawned < this.target; if (this.player.weapon === "拂尘" && combatActive) { this.brushHealTimer += delta; while (this.brushHealTimer >= 3000) { this.brushHealTimer -= 3000; this.healPlayer(this.player.stats.maxHp * .01); } } else this.brushHealTimer = 0; for (const enemy of this.enemies) { if (!enemy.alive) continue; enemy.cooldown -= delta; enemy.hitFlash = Math.max(0, enemy.hitFlash - delta); if (enemy.burn > 0) { const activeDelta = Math.min(delta, enemy.burn); enemy.burnTick += activeDelta; enemy.burn = Math.max(0, enemy.burn - delta); while (enemy.burnTick >= 1000) { enemy.burnTick -= 1000; this.damageEnemy(enemy, Math.max(1, Math.floor(enemy.hpMax * .01)), false, false); } if (!enemy.alive) continue; }
    ...
    if (this.defeated === this.target && !this.items.some((item) => item.kind === "chest")) this.spawnRewards();
  }
  ...
  private healPlayer(amount: number): void { const before = this.player.hp; this.player.heal(amount); const recovered = Math.round(this.player.hp - before); if (recovered > 0) this.floatText(this.player.position, `+${recovered}`, "#62e8a5"); }
  private damageEnemy(enemy: Enemy, amount: number, crit: boolean, burn = false): void { if (!enemy.alive) return; const dead = enemy.hurt(amount, burn); this.floatText(enemy.position, `${crit ? "暴击 " : ""}${amount}`, crit ? "#f4d561" : "#d9ffff"); if (dead) { this.defeated++; if (this.rng.next() < .1) this.items.push(this.makeItem("medpack", enemy.position.clone())); } }
